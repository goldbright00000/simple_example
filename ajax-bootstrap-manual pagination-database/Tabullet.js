...
(function ($) {
    $.fn.tabullet = function (options) {
        ...
        $(metadata).each(function (i, v) {
            ...
            if (v.readonly !== 'true') {
                if (index==4) {
                $('<td/>').html('<input type="date" name="' + v.map + '" class="' + options.textClass + '"/>').appendTo(insertRow);
                }
                // else if(index==6){
                // $('<td/>').html('<select><option value="volvo" name="' + v.map + '" value="M">M</option><option value="saab" value="M">W</option></select>').appendTo(insertRow);
                // }
                else{
                    ...
                    td.find('button').click(function (event) {
                        ...
                        $(rowChildren).each(function (ri, rv) {
                            ...
                            if ($(rv).attr('data-tabullet-readonly') !== 'true') {                       
                                if (index==4) {
                                    $(rv).html('<input type="date" name="' + mapName + '" value="' + v[mapName] + '" class="' + options.textClass + '"/>');                                    
                                }
                                // else if(index==6){
                                // $(rv).html('<select><option value="volvo">M</option><option value="saab">W</option></select>');                                    
                                // }
                                else{
